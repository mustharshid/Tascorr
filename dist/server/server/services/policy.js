...
class PolicyService {
     ...
     */
    static async canViewTask(user, taskId) {
        // NNR-1: Absolute tenant isolation enforcement
        const task = await db_js_1.default.task.findFirst({
            where: { id: taskId },
            include: {
                assignments: true,
                department: true,
        ...
        if (task.tenantId !== user.tenantId)
            return false; // NNR-1 violation prevention
        ...
        const isAssignee = task.assignments.some(a => a.userId === user.userId && a.isActive);
        if (isAssignee)
            return true;
        ...
        for (const assignment of task.assignments.filter(a => a.isActive)) {
            const assignee = await db_js_1.default.user.findUnique({
     ...
     */
    static async canAssignTask(user, assigneeId) {
        ...
        if (!assignee)
            return false;
        if (assignee.tenantId !== user.tenantId)
            return false;
        ...
        if (assignee.managerId === user.userId)
            return true;
        // Rule 3: Valid cross-department authorization request exists and is active (approved, not expired)
        const activeCrossAuth = await db_js_1.default.crossDeptAuthorization.findFirst({
            where: {
     ...
     */
    static async canReassignTask(user, taskId) {
        if (user.rankLevel === 0)
            return true;
