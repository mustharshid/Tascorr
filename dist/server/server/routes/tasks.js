 ...
 */
router.post('/', auth_middleware_js_1.authenticateSession, async (req, res) => {
    ...
    if (!validator_js_1.Validator.validateDate(dueDate)) {
    ...
    try {
        ...
        for (const assigneeId of assigneeIds) {
            const isAllowed = await policy_js_1.PolicyService.canAssignTask(req.user, assigneeId);
            if (!isAllowed) {
        ...
        const task = await db_js_1.default.$transaction(async (tx) => {
...
router.get('/', auth_middleware_js_1.authenticateSession, async (req, res) => {
    ...
    try {
        ...
        const tasks = await db_js_1.default.task.findMany({
            where: whereClause,
            include: {
                assignments: {
                    where: { isActive: true },
        ...
        for (const task of tasks) {
            const isAllowed = await policy_js_1.PolicyService.canViewTask(req.user, task.id);
            if (isAllowed) {
 ...
 */
router.get('/workload', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const tenantId = req.user.tenantId;
    try {
        const assignments = await db_js_1.default.taskAssignment.findMany({
            where: {
                isActive: true,
                task: {
                    tenantId: tenantId,
...
router.get('/:id', auth_middleware_js_1.authenticateSession, async (req, res) => {
    ...
    try {
        const isAllowed = await policy_js_1.PolicyService.canViewTask(req.user, taskId);
        if (!isAllowed) {
            return res.status(403).json({ error: 'Access denied. You are not authorized to view this task.' });
        }
        const task = await db_js_1.default.task.findUnique({
            where: { id: taskId },
            include: {
                assignments: {
                    include: {
 ...
 */
router.patch('/:id/status', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    ...
    try {
        ...
        if (status === 'Completed') {
            ...
            if (task.departmentId) {
                const dept = await db_js_1.default.department.findUnique({ where: { id: task.departmentId } });
        ...
        const updatedTask = await db_js_1.default.$transaction(async (tx) => {
            const t = await tx.task.update({
                where: { id: taskId },
                data: { status },
            });
            await audit_js_1.AuditService.logAction(req.user.tenantId, req.user.userId, 'TASK_STATUS_CHANGE', 'Task', taskId, { from: task.status, to: status }, tx);
...
router.patch('/:id', auth_middleware_js_1.authenticateSession, async (req, res) => {
    ...
    try {
        ...
        const updatedTask = await db_js_1.default.$transaction(async (tx) => {
            ...
            await audit_js_1.AuditService.logAction(req.user.tenantId, req.user.userId, 'TASK_EDIT', 'Task', taskId, { updates }, tx);
            return t;
        });
 ...
 */
router.post('/:id/reassign', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    const { targetAssigneeId, reason } = req.body;
    if (isNaN(taskId)) {
        return res.status(400).json({ error: 'Invalid task ID.' });
    }
    const missing = validator_js_1.Validator.validateRequired(req.body, ['targetAssigneeId', 'reason']);
    if (missing.length > 0) {
    ...
    try {
        ...
        const isAuthorized = await policy_js_1.PolicyService.canReassignTask(req.user, taskId);
        if (!isAuthorized) {
            return res.status(403).json({ error: 'Access denied. Only creator, department head, or admin can reassign.' });
        }
        const isAssignable = await policy_js_1.PolicyService.canAssignTask(req.user, targetAssigneeId);
        if (!isAssignable) {
 ...
 */
router.post('/:id/blockers', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    ...
    try {
        ...
        const isAssignee = task.assignments.some(a => a.userId === req.user.userId && a.isActive);
        if (!isAssignee) {
 ...
 */
router.patch('/:id/blockers/:blockerId/resolve', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    ...
    const missing = validator_js_1.Validator.validateRequired(req.body, ['resolutionComment']);
    if (missing.length > 0) {
        return res.status(400).json({ error: `Missing mandatory parameter fields: ${missing.join(', ')}` });
    }
    try {
        ...
        const blocker = await db_js_1.default.blocker.findUnique({
            where: { id: blockerId },
        });
        ...
        const resolvedBlocker = await db_js_1.default.$transaction(async (tx) => {
 ...
 */
router.post('/:id/comments', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    const { content } = req.body;
    if (isNaN(taskId)) {
        return res.status(400).json({ error: 'Invalid task ID format.' });
    }
    const missing = validator_js_1.Validator.validateRequired(req.body, ['content']);
    if (missing.length > 0) {
        return res.status(400).json({ error: `Missing mandatory parameter fields: ${missing.join(', ')}` });
    }
    try {
 ...
 */
router.patch('/:id/subtasks/:subtaskId', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    ...
    try {
        ...
        const subtask = await db_js_1.default.subtask.findUnique({
            where: { id: subtaskId },
        });
        if (!subtask || subtask.taskId !== taskId) {
            return res.status(404).json({ error: 'Subtask not found.' });
        }
        const updated = await db_js_1.default.$transaction(async (tx) => {
...
router.delete('/:id', auth_middleware_js_1.authenticateSession, async (req, res) => {
    const taskId = Number(req.params.id);
    if (isNaN(taskId)) {
        return res.status(400).json({ error: 'Invalid task ID.' });
    }
