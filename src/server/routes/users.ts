...
router.post('/', authenticateSession, requireAdmin, async (req: Request, res: Response) => {
  ...
  const missing = Validator.validateRequired(req.body, ['email', 'password', 'firstName', 'lastName', 'rankId']);
  if (missing.length > 0) {
 ...
 */
router.get('/ranks', authenticateSession, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenantId;
  try {
 ...
 */
router.post('/ranks', authenticateSession, requireAdmin, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenantId;
 ...
 */
router.get('/tenant/details', authenticateSession, requireAdmin, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenantId;
  try {
    const tenant = await prisma.tenant.findUnique({
      where: { id: tenantId },
 ...
 */
router.patch('/ranks/:rankId', authenticateSession, requireAdmin, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenantId;
  const rankId = Number(req.params.rankId);
  if (isNaN(rankId)) return res.status(400).json({ error: 'Invalid rank ID.' });
  ...
  try {
    ...
    const updateData: any = {};
    if (title !== undefined && title.trim().length > 0) updateData.title = title.trim();
    if (level !== undefined) {
      const levelInt = Number(level);
      if (isNaN(levelInt) || levelInt < 0) {
        return res.status(400).json({ error: 'Authority level must be a non-negative integer.' });
      ...
      const conflict = await prisma.rank.findFirst({
    ...
    const updated = await prisma.rank.update({ where: { id: rankId }, data: updateData });
 ...
 */
router.delete('/ranks/:rankId', authenticateSession, requireAdmin, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenantId;
  const rankId = Number(req.params.rankId);
  if (isNaN(rankId)) return res.status(400).json({ error: 'Invalid rank ID.' });
