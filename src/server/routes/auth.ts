...
router.post('/login', async (req: Request, res: Response) => {
  ...
  const missing = Validator.validateRequired(req.body, ['email', 'password']);
  if (missing.length > 0) {
    return res.status(400).json({ error: `Missing mandatory parameter fields: ${missing.join(', ')}` });
  }
  ...
  try {
    ...
    const sessionPayload: IAuthSession = {
      userId: user.id,
 ...
 */
router.get('/session', authenticateSession, async (req: Request, res: Response) => {
  try {
